class VideoController {
  constructor() {
    ...
    this.updateInterval = 1000 / 60; // 60fps로 UI 업데이트
  ...
  moveFrame(frames) {
    ...
    this.video.currentTime = Math.max(
      0,
      Math.min(this.video.duration, this.video.currentTime + frames * frameTime)
    );
    this.pause();
  }
  ...
  // 수정: 타임라인 마커 업데이트 로직 개선
  updateTimelineMarker() {
    if (!this.video.src || !this.timelineMarker) {
      return;
    }
    ...
    const updateMarker = () => {
      ...
      if (now - this.lastUpdateTime >= this.updateInterval) {
        ...
        // 위치 업데이트가 필요한 경우에만 DOM 업데이트
        const currentLeft = parseFloat(this.timelineMarker.style.left) || 0;
        if (Math.abs(currentLeft - percentage) > 0.1) {
      ...
      // 재생 중일 때만 RAF 계속 실행
      if (this.isPlaying) {
        this.animationFrameId = requestAnimationFrame(updateMarker);
      }
    };
  ...
  moveFrame(frames) {
    ...
    this.video.currentTime = Math.max(
      0,
      Math.min(this.video.duration, targetTime)
    );
    this.pause();
  ...
  updateTimelineMarker() {
    ...
    if (
      !this.timelineMarker.style.left ||
      percentage !== parseFloat(this.timelineMarker.style.left)
    ) {
